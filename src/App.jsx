...
function PrimaryBtn({ onClick, disabled, children }) {
  return (
    <button disabled={disabled} onClick={onClick}
      style={{ background: disabled ? "#ccc" : ORANGE, color: "white", width: "100%", border: "none", borderRadius: 8, padding: "10px 0", fontSize: 14, fontWeight: 700, cursor: disabled ? "not-allowed" : "pointer", fontFamily: "'Nunito',sans-serif", opacity: disabled ? 0.6 : 1 }}>
      {children}
    </button>
  );
}
...
function BackBtn({ onClick }) {
  return (
    <button onClick={onClick}
      style={{ flex: 1, border: "1px solid #e5e7eb", background: "white", color: "#4b5563", borderRadius: 8, padding: "10px 0", fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "'Nunito',sans-serif" }}>
      ← Back
    </button>
  );
}
...
function Tab({ active, onClick, children }) {
  return (
    <button onClick={onClick}
      style={{ background: active ? ORANGE : "transparent", color: active ? "white" : "#6b7280", border: active ? "none" : "1px solid #e5e7eb", borderRadius: 8, padding: "8px 16px", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "'Nunito',sans-serif" }}>
      {children}
    </button>
  );
}
...
function StepDot({ n, current }) {
  return (
    <div style={{ width: 28, height: 28, borderRadius: "50%", background: current >= n ? ORANGE : "#e5e7eb", color: current >= n ? "white" : "#9ca3af", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700 }}>
      {n}
    </div>
  );
}

function SectionTitle({ children, color = NAVY }) {
  return (
    <p style={{ fontSize: 11, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.05em", color, marginBottom: 12 }}>{children}</p>
  );
}

function AdditionalIncomeInputs({ incomes, update }) {
  return (
    <div>
...
export default function App() {
  ...
  function DrawdownSection({ drawdownData, depYears, depYearsBase, annWithdrawal, addIncomes, title, subtitle }) {
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 32 }}>
              {[1, 2, 3, 4].map((s, i) => (
                <div key={s} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <span style={{ fontSize: 12, fontWeight: 700, color: step === s ? "#374151" : "#9ca3af" }}>{stepLabels[i]}</span>
                          ...
                          <tr key={i} style={{ borderBottom: "1px solid #f9fafb" }}>
                            <td style={{ padding: "12px 0", fontWeight: 700, color: "#374151" }}>
                              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                <div style={{ width: 8, height: 8, borderRadius: "50%", background: COLORS[i] }} />{LABELS[i]}
                              </div>
                            </td>
